...
> **Can we predict a track's popularity from its audio characteristics alone — independent of how famous the artist is — and use the gap between prediction and reality to surface "undiscovered bangers"?**
...
2. **Deduplicating releases.** We sorted by `popularity` (descending) and kept one representative row per `(track_name, artists)` pair, so the surviving record reflects the track's best-performing release. Ties were broken deterministically by keeping the first occurrence.
...
Track popularity is heavily right-skewed, with a pronounced spike at 0 representing tracks with virtually no streaming activity. Beyond this zero-popularity mass, the distribution shows a secondary concentration between roughly 40 and 70, hinting at a bimodal structure in which tracks tend to be either largely undiscovered or moderately mainstream.
...
To make this missingness **MAR** instead, we would want access to Spotify's internal **API indexing status** or **artist streaming-activity logs**. With those, we could explain the missingness using observable metadata (whether a profile was fully indexed) rather than the unobserved follower count, turning the mechanism from NMAR into MAR.
...
The plot shows the empirical distribution of the TVD statistic under the null (genre labels shuffled), with the observed TVD marked in red. The observed value falls far outside the null distribution, visually confirming the strong dependence of `tempo` missingness on `track_genre`.
...
- **Alternative hypothesis (Hₐ):** Among hip-hop tracks, the mean popularity of explicit and non-explicit tracks is _different_. (Two-sided — we do not assume a direction in advance.)
- **Test statistic:** the absolute difference in mean popularity between the two groups.
- **Significance level:** α = 0.05.

The observed difference in means was **−14.548** (explicit minus non-explicit), meaning explicit hip-hop tracks were on average about 14.5 popularity points _lower_. The permutation test returned **p < 0.001**, so we **reject the null hypothesis**.
...
Our evaluation metric is **RMSE (Root Mean Squared Error)**. RMSE is appropriate because `popularity` is continuous and on a fixed 0–100 scale, RMSE is expressed in the same units as the target (so an error is directly interpretable as "off by N popularity points"), and squaring penalizes the large misses that matter most for our use case. We prefer it over alternatives like MAE because the serendipity logic depends on identifying the _large_ gaps between prediction and reality, which RMSE emphasizes.

**Features available at the time of prediction:** all the audio characteristics (danceability, energy, loudness, etc.) are intrinsic to a track and known the moment it is uploaded, so they are valid inputs. We deliberately **exclude `followers` and `fame_score`**: although they are technically available, feeding artist fame into the model would simply re-encode the filter bubble we are trying to see past. Keeping the model fame-blind is what makes its residuals meaningful as a serendipity signal.

---

## Baseline Model

Our baseline is a **Linear Regression** model implemented in a single scikit-learn `Pipeline`, trained on two features:

- **`track_genre`** — _nominal_, one-hot encoded with `OneHotEncoder(drop='first')`.
- **`danceability`** — _quantitative_, passed through as-is.
...
**Engineered feature 1 — Speechiness binarization (`Binarizer`, threshold = 0.33).** Spotify documents that a `speechiness` below 0.33 indicates pure music while values above it begin mixing in spoken word. From a DGP standpoint, the jump from "music" to "music + speech" is a structural threshold, not a smooth gradient — so a clean binary flag captures the listener-relevant distinction far better than the raw continuous value.
...
We also impute missing `tempo` with a custom `GroupMedianImputer` that learns genre-wise medians from the training fold only (preventing leakage), reflecting the DGP fact that tempo ranges are tightly genre-dependent.
...
- **Evaluation metric:** RMSE within each group (a regression metric — precision/recall do not apply).
